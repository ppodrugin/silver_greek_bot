...
1️⃣ /add_words - Добавление слов в словарь
   Формат: /add_words Название урока
   Затем отправьте слова в формате "слово,перевод" или многострочный текст

2️⃣ /training - Тренировка слов
   Бот будет показывать слова из словаря, вы произносите их на греческом
   Можно указать урок: /training Название урока
...
5️⃣ /info - Показать информацию о версии бота и статистику
   Можно указать урок: /info Название урока

6️⃣ /lessons - Показать список всех уроков

7️⃣ /get_words - Экспортировать все слова из словаря в формате CSV
    ...
    try:
        ...
        if is_correct:
            ...
            await update.message.reply_text(
                ...
                f"Можете попробовать еще раз или отправить новый текст."
            )
            # Не сбрасываем режим, чтобы можно было читать текст несколько раз
        else:
...
def main():
    ...
    # Команды (только латиница, Telegram не поддерживает кириллицу в командах)
