...
### Способ 2: Через скрипт миграции

1. Откройте файл `users.txt` в папке бота
2. Добавьте `user_id` пользователя (каждая строка - один ID)
3. Запустите скрипт миграции: `python3 migrate_users.py`
4. После миграции файл `users.txt` можно удалить
...
1. Пользователь получает свой ID через @userinfobot (например, `123456789`)
2. Администратор добавляет `123456789` в файл `users.txt`
...
- Статистика хранится в таблице `word_statistics` в базе данных SQLite
